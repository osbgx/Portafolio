...
Si el formulario o analytics requieren secrets en build time, configurarlos en **Settings → Secrets and variables → Actions** (ej. `PUBLIC_WEB3FORMS_ACCESS_KEY`).

## Debug

- Revisar logs del job Build si falla `generate-og-image.mjs` (necesita `public/me.webp`).
